...
export function searchErp(state: AppState, q: string, role: Role = "admin"): SearchHit[] {
  ...
  for (const s of state.students.filter((x) => !x.deletedAt)) {
    ...
    if (hay.includes(needle)) {
      hits.push({
        href: staffOnly ? "/staff/students" : `/app/students/${s.id}`,
        title: s.name,
        subtitle: s.rollNo,
        kind: "Student",
      });
  ...
  for (const n of state.notices.filter((x) => !x.archived)) {
    if (`${n.title} ${n.body}`.toLowerCase().includes(needle)) {
      hits.push({ href: staffOnly ? "/staff/notices" : "/app/alerts", title: n.title, subtitle: n.severity, kind: "Notice" });
    }
  }
  if (!staffOnly) {
    ...
    for (const t of state.timetable) {
      const name = getCourseName(state, t.courseId);
      if (name.toLowerCase().includes(needle) || t.room.toLowerCase().includes(needle)) {
        hits.push({ href: "/staff/classes", title: name, subtitle: `${t.day} · ${t.period}`, kind: "Class" });
      }
    }
  }
