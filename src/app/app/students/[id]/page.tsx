...
export default function StudentFilePage() {
  ...
  const route = state.routes.find((r) => r.id === st.busRouteId);
    ...
    <Guard module="students">
      ...
      <Tabs defaultValue="personal">
        ...
        <TabsContent value="academic" className="grid gap-3 sm:grid-cols-2">
          <Info title="Programme" value={course?.name} />
          <Info title="Section" value={section?.name} />
        ...
        <TabsContent value="attendance">
          {att.length === 0 ? <p className="text-sm text-muted-foreground">No attendance marked.</p> : att.slice(0, 30).map((a) => (
            <p key={a.id} className="text-sm">{a.date} · {a.status}</p>
          ))}
        </TabsContent>
        <TabsContent value="exams">
          {marks.length === 0 ? <p className="text-sm text-muted-foreground">No marks yet.</p> : marks.map((m) => {
            const ex = state.exams.find((e) => e.id === m.examId);
            return <p key={m.id} className="text-sm">{ex?.name ?? m.examId} · {m.marks}/{ex?.maxMarks ?? "—"}</p>;
          })}
        </TabsContent>
        <TabsContent value="fees">
          {fees.length === 0 ? <p className="text-sm text-muted-foreground">No fee bills.</p> : fees.map((f) => (
            <p key={f.id} className="text-sm">{f.term} · ₹{f.amount.toLocaleString("en-IN")} · <Badge variant="outline">{f.status}</Badge></p>
          ))}
        </TabsContent>
        <TabsContent value="library">
          {books.length === 0 ? <p className="text-sm text-muted-foreground">No library issues.</p> : books.map((c) => (
            <p key={c.id} className="text-sm">{state.books.find((b) => b.id === c.bookId)?.title} · due {c.dueOn} {c.returnedOn ? `· returned ${c.returnedOn}` : ""}</p>
          ))}
        </TabsContent>
        <TabsContent value="transport">
          <Info title="Route" value={route ? `${route.name} · ${route.vehicleNo}` : "Not assigned"} />
        </TabsContent>
        <TabsContent value="history">
          {logs.length === 0 ? <p className="text-sm text-muted-foreground">No audit rows for this file on this device.</p> : logs.map((l) => (
            <p key={l.id} className="text-sm">{l.at} · {l.action} · {l.details}</p>
          ))}
        </TabsContent>
      </Tabs>
    </Guard>
