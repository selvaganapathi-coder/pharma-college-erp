...
export function PortalShell({
  ...
  return (
    <div className={`min-h-screen bg-background ${dense ? "md:grid md:grid-cols-[248px_1fr]" : "md:grid md:grid-cols-[228px_1fr]"}`}>
      ...
      <div className={`min-w-0 ${bottomNav ? "pb-20 md:pb-0" : ""}`}>
        <header className="sticky top-0 z-30 flex items-center gap-3 border-b border-border/70 bg-white/95 px-3 py-3 backdrop-blur md:px-6">
          ...
          {search ? (
            ...
            <div className="min-w-0 flex-1">
              ...
              <p className="truncate text-[11px] text-muted-foreground">{COLLEGE_SYSTEM}</p>
            </div>
          )}
      ...
      {bottomNav ? (
        <nav className="fixed right-0 bottom-0 left-0 z-30 grid grid-cols-5 border-t border-border bg-white/95 px-1 py-1 md:hidden" aria-label="Primary">
          {bottomNav.map((item) => {
            const active = pathname === item.href || (item.href !== portalHome(user.role) && pathname.startsWith(item.href));
            const Icon = item.icon;
            return (
